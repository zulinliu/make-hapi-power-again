...
- [ ] **Phase T1: 功能冒烟测试** — 启动 dev server，端到端验证核心流程
- [ ] **Phase T2: 补测试** — 核心模块测试覆盖率提升到 80%
- [ ] **Phase T3: 前端 UI 打磨** — 功能页面对齐设计规格
- [ ] **Phase T4: 构建发布** — 全量构建验证 + v0.1 tag
...
**Depends on**: Phase T1
**Success Criteria**:
  1. Hub 核心路由（sessions/messages/git/plugins/skills/changes/undo/timeline/share）有集成测试
...
**Success Criteria**:
  ...
  2. `bun run build:single-exe` 单文件可执行程序构建成功
  3. 构建产物可运行
  4. 打 v0.1 tag
  5. Push tag 到远程
