...
v0.18.0 围绕五个产品模块组织。每个模块都有明确入口、实际产出，以及 Agent 或 Provider 不支持高级能力时的安全降级。

| 步骤 | 模块 | 入口 | 作用 |
|------|------|------|------|
| 接入 | **模型星桥** | 设置 -> 模型星桥 | 统一管理 Provider、健康检测、模型发现、能力缓存、密钥加密和 Agent 默认路由。 |
| 驾驶 | **引导光标** | Agent 思考中的会话输入框 | 支持把紧急纠偏作为隔离引导消息送达；不满足能力条件时自动回到普通队列，不丢文字。 |
| 观测 | **上下文脉冲** | 会话状态栏 | 显示 `上下文：n%`、token/cache 详情、来源诊断和不可用原因，避免已用量长期显示 0 或无解释。 |
| 追踪 | **Git 脉络** | 会话 -> Git | 把分支态势、变更地图、Diff 预览、提交篮、同步中心和危险操作确认合并到独立 Git 页面。 |
| 沉淀 | **会话织锦** | 会话 -> 会话织锦 | 提供服务端大纲、Markdown 导出、后台设计方案提炼和可下载会话资产。 |

会话织锦有两条不同产出路径：**导出**是基于已存会话记录的确定性 Markdown 生成；**提炼**会在后台调用当前会话 Agent 正在使用的供应商 API 模型，综合完整对话生成可复用的“设计方案”Markdown，不会把提炼过程插入主线聊天。

详细行为、安全边界和代码入口见 [v0.18.0 五个功能模块](./docs/v0.18-five-modules.md)。
...
**会话织锦（Session Loom）** — 将“大纲”升级为独立会话资产工作台，一键导出确定性 Markdown，过滤噪音，并通过当前会话 Agent 的模型路线在后台提炼设计方案。
...
导出是确定性 Markdown 生成；提炼是单独的后台大模型调用，会生成“设计方案”Markdown 资产，不打断当前主线会话。
...
- [v0.18.0 五个功能模块](./docs/v0.18-five-modules.md) — 模型星桥、引导光标、上下文脉冲、Git 脉络、会话织锦
