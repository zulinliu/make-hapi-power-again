        ...
        <div className="flex flex-col gap-2 px-3 py-3">
            ...
            {props.isLoading ? (
                <div className="flex flex-col gap-2" data-testid="opencode-model-loading">
                    <div className="flex items-center gap-2 text-xs text-(--hp-text-tertiary)">
                        <span className="inline-block h-3 w-3 animate-spin rounded-full border-2 border-(--hp-border) border-t-(--hp-primary)" />
                    ...
                    <div className="flex flex-col gap-1.5" aria-hidden="true">
                        {[0, 1, 2, 3].map((i) => (
                            <div
                                key={i}
                                className="h-7 w-full animate-pulse rounded-[var(--hp-radius-sm,6px)] bg-(--hp-surface-1)"
                            />
                        ))}
                    </div>
                </div>
                ...
                <div className="flex flex-col gap-2" data-testid="opencode-model-error">
                    ...
                    {props.onRetry ? (
                        <button
                            ...
                            className="self-start rounded-[var(--hp-radius-sm,6px)] border border-(--hp-border) px-2 py-1 text-xs text-(--hp-primary) hover:bg-(--hp-surface-1) transition-colors"
                        >
                ...
                <div className="text-xs text-(--hp-text-tertiary)" data-testid="opencode-model-empty">
                    ...
                    {props.availableModels.map((model) => {
                        ...
                        return (
                            <button
                                ...
                                className="flex w-full items-center gap-2 rounded-[var(--hp-radius-sm,6px)] px-2 py-2 text-left text-sm transition-colors hover:bg-(--hp-surface-1)"
                            >
                                <div
                                ...
                                <span className={`flex-1 truncate ${isSelected ? 'text-(--hp-primary)' : 'text-(--hp-text-secondary)'}`}>
                                    {model.name ?? model.modelId}
                                </span>
                                {isDefault ? (
                                    <span className="rounded-[var(--hp-radius-xs,4px)] border border-(--hp-border) px-1.5 py-0.5 text-[10px] uppercase tracking-wide text-(--hp-text-tertiary)">
