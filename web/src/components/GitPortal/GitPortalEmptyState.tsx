...
export function GitPortalEmptyState({ variant, error, onRetry }: GitPortalEmptyStateProps) {
  ...
  if (variant === 'cloneFailed') {
    return (
      <div className="gp-empty-state flex flex-col items-center justify-center py-8 px-4 text-center">
        <svg
          className="w-10 h-10 text-[var(--hp-danger)] mb-3"
          viewBox="0 0 24 24"
          fill="none"
          stroke="currentColor"
          strokeWidth="2"
        ...
        {error && (
          <p className="text-sm text-[var(--hp-danger)] mb-3 max-w-[280px] break-words">
            {error}
          </p>
        )}
        {onRetry && (
          <button
            type="button"
            className="gp-retry-btn px-4 py-1.5 text-sm rounded-md bg-[var(--hp-primary)] text-[var(--hp-primary-text)] hover:bg-[var(--hp-primary-hover)] transition-colors"
            onClick={onRetry}
          >
