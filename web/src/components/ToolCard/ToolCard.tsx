...
function ElapsedView(props: { from: number; active: boolean }) {
    ...
    return (
        <span className="font-mono text-xs text-[var(--hp-text-tertiary)]">
            {elapsed.toFixed(1)}s
        </span>
    )
}
            ...
            <div className="flex flex-col gap-1">
                {visible.map((child) => (
                    <div key={child.id} className="flex items-center gap-2">
                        <div className="min-w-0 flex-1 font-mono text-xs text-[var(--hp-text-tertiary)]">
                ...
                {remaining > 0 ? (
                    <div className="text-xs text-[var(--hp-text-tertiary)] italic">
                        (+{remaining} more)
                    </div>
                ) : null}
            </div>
...
export function toolStatusColorClass(state: ToolCallBlock['tool']['state']): string {
    if (state === 'completed') return 'text-[var(--hp-success)]'
    if (state === 'error') return 'text-[var(--hp-danger)]'
    if (state === 'pending') return 'text-[var(--hp-warning)]'
    return 'text-[var(--hp-text-tertiary)]'
}
...
export function ToolDetailDialogContent(props: {
    ...
    return (
        <div className="mt-3 flex max-h-[75vh] flex-col gap-4 overflow-auto">
            <div>
                <div className="mb-1 text-xs font-medium text-[var(--hp-text-tertiary)]">
            ...
            {!isQuestionToolWithAnswers ? (
                <div>
                    <div className="mb-1 text-xs font-medium text-[var(--hp-text-tertiary)]">{t('tool.result')}</div>
...
function ToolCardInner(props: ToolCardProps) {
        ...
        <div className="flex items-center justify-between gap-3">
            <div className="min-w-0 flex flex-1 flex-col gap-1">
                <div className="min-w-0 flex items-center gap-2">
                    <div className="shrink-0 flex h-3.5 w-3.5 items-center justify-center text-[var(--hp-primary)] leading-none">
                        {presentation.icon}
                    </div>
                    <CardTitle className={cn(
                        'min-w-0 text-sm font-medium leading-tight text-[var(--hp-text-primary)]',
                ...
                {subtitle ? (
                    <CardDescription className={cn(
                        'font-mono text-xs text-[var(--hp-text-secondary)]',
            ...
            <div className={cn(
                'flex shrink-0 items-center gap-2 self-center text-[var(--hp-text-tertiary)]',
                ...
                <span className="text-[var(--hp-text-tertiary)]">
                    <DetailsIcon />
                </span>
            </div>
        </div>
    ...
    return (
        <Card className={cn(
            'overflow-hidden rounded-[var(--hp-radius-md)] bg-[var(--hp-surface-1)] shadow-none hover:bg-[var(--hp-surface-2)] transition-colors',
            props.block.tool.state === 'running' && 'border-t-2 border-t-[var(--hp-primary)]',
            props.block.tool.state === 'error' && 'bg-[var(--hp-danger-subtle)]'
        )}>
                ...
                <Dialog open={detailsOpen} onOpenChange={setDetailsOpen}>
                    <DialogTrigger asChild>
                        <button
                            type="button"
                            className={cn(
                                'w-full text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--hp-primary)]',
            ...
            {hasBody ? (
                <CardContent className="px-3 pb-3 pt-1">
                    ...
                    {showInline ? (
                        CompactToolView ? (
                            <div
                                className="mt-3 cursor-pointer rounded-xl focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--hp-primary)]"
                                role="button"
                                tabIndex={0}
                            ...
                            <div className="mt-3 flex flex-col gap-3">
                                <div
                                    className="cursor-pointer rounded-xl focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--hp-primary)]"
                                    role="button"
                                    tabIndex={0}
                                    onClick={openDetailsFromInlinePreview}
                                    onKeyDown={openDetailsFromInlinePreviewKeyDown}
                                >
                                    <div className="mb-1 text-xs font-medium text-[var(--hp-text-tertiary)]">{t('tool.input')}</div>
