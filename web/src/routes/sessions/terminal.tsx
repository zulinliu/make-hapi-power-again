...
function ConnectionIndicator(props: { status: 'idle' | 'connecting' | 'connected' | 'error' }) {
    ...
    const colorClass = isConnected
        ? 'bg-[var(--hp-success)]'
        : isConnecting
          ? 'bg-[var(--hp-warning)] animate-pulse'
          : 'bg-[var(--hp-text-tertiary)]'
...
function QuickKeyButton(props: {
    ...
    return (
        <button
            ...
            className={`flex-1 border-l border-[var(--hp-border)] px-2 py-1.5 text-xs font-medium text-[var(--hp-text-primary)] transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--hp-primary)] focus-visible:ring-inset disabled:cursor-not-allowed disabled:opacity-50 disabled:hover:bg-transparent first:border-l-0 active:bg-[var(--hp-surface-1)] sm:px-3 sm:text-sm ${
                isActive ? 'bg-[var(--hp-primary)] text-[var(--hp-surface-0)]' : 'hover:bg-[var(--hp-surface-1)]'
            }`}
        ...
        <div className="flex h-full min-h-0 flex-col">
            {session.active ? null : (
                <div className="px-3 pt-3">
                    <div className="mx-auto w-full max-w-content rounded-[var(--hp-radius-sm)] bg-[var(--hp-surface-1)] p-3 text-sm text-[var(--hp-text-tertiary)]">
            ...
            {errorMessage ? (
                <div className="mx-auto w-full max-w-content px-3 pt-3">
                    <div className="rounded-[var(--hp-radius-sm)] border border-[var(--hp-danger)] bg-[var(--hp-danger-subtle)] p-3 text-xs text-[var(--hp-danger)]">
                        {errorMessage}
                    </div>
                </div>
            ) : null}
            ...
            {exitInfo ? (
                <div className="mx-auto w-full max-w-content px-3 pt-3">
                    <div className="rounded-[var(--hp-radius-sm)] border border-[var(--hp-border)] bg-[var(--hp-surface-1)] p-3 text-xs text-[var(--hp-text-tertiary)]">
                        {exitInfo.code !== null
            ...
            <div className="flex-1 min-h-0 overflow-hidden bg-[var(--hp-surface-0)] relative">
                ...
                <div className="mx-auto h-full w-full max-w-content p-3">
                    {terminalSupported ? (
                        ...
                        <div className="flex h-full items-center justify-center rounded-[var(--hp-radius-sm)] border border-[var(--hp-border)] bg-[var(--hp-surface-1)] p-4 text-sm text-[var(--hp-text-tertiary)]">
                            {t('terminal.unsupportedWindows')}
                        </div>
                    )}
                </div>
            ...
            <div className="bg-[var(--hp-surface-0)] border-t border-[var(--hp-border)] pb-[env(safe-area-inset-bottom)]">
                <div className="mx-auto w-full max-w-content px-3">
                    <div className="flex flex-col gap-2 py-2">
                        <button
                            ...
                            className="w-full rounded-[var(--hp-radius-sm)] border border-[var(--hp-border)] bg-[var(--hp-surface-1)] px-3 py-2 text-sm font-medium text-[var(--hp-text-primary)] transition-colors hover:bg-[var(--hp-surface-1)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--hp-primary)] disabled:cursor-not-allowed disabled:opacity-50"
                        >
                            {t('button.paste')}
                        </button>
                        {QUICK_INPUT_ROWS.map((row, rowIndex) => (
                            <div
                                ...
                                className="flex items-stretch overflow-hidden rounded-[var(--hp-radius-sm)] bg-[var(--hp-surface-1)]"
                            >
                                {row.map((input) => {
                                    const modifier = input.modifier
            ...
            >
                <DialogContent className="max-w-md">
                    ...
                    <textarea
                        ...
                        className="mt-2 min-h-32 w-full resize-y rounded-[var(--hp-radius-sm)] border border-[var(--hp-border)] bg-[var(--hp-surface-0)] p-2 text-sm focus:outline-none focus:ring-2 focus:ring-[var(--hp-primary)]"
                        autoCapitalize="none"
                        autoCorrect="off"
                    />
