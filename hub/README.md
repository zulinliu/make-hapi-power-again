...
| `PORT` | HTTP 监听端口 | `3000` |
| `HUB_TOKEN` | Hub 认证令牌 | - |
| `HAPI_POWER_LISTEN_HOST` | HTTP 绑定地址 | `127.0.0.1` |
| `ALLOWED_ORIGINS` | CORS 允许来源 | - |
| `DATA_DIR` / `HAPI_POWER_HOME` | 数据目录 | `~/.hapi-power` |
| `DB_PATH` | SQLite 数据库路径 | `DATA_DIR/hapi-power.db` |
| `OPENAI_API_KEY` | Whisper 语音转录 | - |
| `VAPID_PUBLIC_KEY` | Web Push 公钥 | - |
| `VAPID_PRIVATE_KEY` | Web Push 私钥 | - |
| `VAPID_SUBJECT` | Web Push 联系邮箱/URL | - |
| `HAPI_POWER_RELAY_API` | 中继 API 域名 | - |
