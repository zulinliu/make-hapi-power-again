...
// Pinned to the major version Hapi Power has been validated against (opencode
...
function buildPluginSource(hookUrl: string, token: string): string {
    ...
    return [
        '// Generated by Hapi Power. Do not edit manually.',
        '',
        `const DEFAULT_HOOK_URL = ${escapedUrl};`,
        `const DEFAULT_HOOK_TOKEN = ${escapedToken};`,
        'const HOOK_URL = process.env.HAPI_POWER_OPENCODE_HOOK_URL || DEFAULT_HOOK_URL;',
        'const HOOK_TOKEN = process.env.HAPI_POWER_OPENCODE_HOOK_TOKEN || DEFAULT_HOOK_TOKEN;',
        '',
...
/**
 * `rootPath` is a Hapi Power-managed directory only when it lives under
 ...
 * that package is resolvable from `<configDir>`. Hapi Power's per-session
...
function ensurePluginRuntime(rootPath: string): void {
    if (!isHapiManagedDir(rootPath)) {
        logger.debug(`[opencode-hook] Skipping plugin runtime materialization for non-Hapi Power dir: ${rootPath}`);
        return;
    }
