...
const RELEASE_URL = 'https://github.com/zulinliu/make-hapi-power-again/releases';
...
function getBinaryPath(platformName = platform, archName = arch) {
    const pkgName = `@hapipower/hapi-power-${platformName}-${archName}`;

    try {
        // Try to find the platform-specific package
        const pkgPath = require.resolve(`${pkgName}/package.json`);
        const binName = platformName === 'win32' ? 'hapi-power.exe' : 'hapi-power';
...
function reportMissingPlatformPackage(platformName = platform, archName = arch, log = console.error) {
    const platformPackage = `@hapipower/hapi-power-${platformName}-${archName}`;
    ...
    log(`  npm install -g @hapipower/hapi-power --registry=${OFFICIAL_NPM_REGISTRY}`);
    log('');
